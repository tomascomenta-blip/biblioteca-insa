...
const BADGES = {
  primer_archivo:    { emoji:"🌱", label:"Primer Paso",        desc:"Subiste tu primer archivo" },
  cinco_archivos:    { emoji:"📚", label:"Bibliófilo",          desc:"Subiste 5 archivos" },
  diez_archivos:     { emoji:"🔥", label:"Colaborador Élite",   desc:"Subiste 10 archivos" },
  primer_like:       { emoji:"❤️",  label:"Querido",             desc:"Tu archivo recibió su primer like" },
  top_descargado:    { emoji:"⬇️",  label:"Lo Más Pedido",       desc:"Tu archivo fue el más descargado" },
  primera_resena:    { emoji:"⭐", label:"Crítico",              desc:"Escribiste tu primera reseña" },
  maestro_asignado:  { emoji:"🎓", label:"Maestro",              desc:"El admin te asignó rol de Maestro" },
};
      ...
      <div style={{ background:bg, border:`1px solid ${border}`, borderRadius:"20px",
        ...
        <div style={{ padding:"1.25rem 1.5rem", borderBottom:`1px solid ${border}` }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
            ...
            <button onClick={onClose} style={{ background:"none", border:"none", color:muted, fontSize:"1.5rem", cursor:"pointer" }}>×</button>
          </div>
        </div>
        {/* Write review */}
        {user && (
        ...
        <div style={{ display:"flex", gap:"0.4rem", flexShrink:0, flexWrap:"wrap", justifyContent:"flex-end" }}>
          ...
          <button onClick={()=>setShowReview(true)} title="Calificar"
            style={{ background:darkMode?"#1A1A1A":"#F5F5F0", border:`1px solid ${border}`,
              color:"#FBBF24", borderRadius:"8px", padding:"0.4rem 0.65rem", fontSize:"0.76rem", cursor:"pointer" }}>
            ⭐
          </button>
          {/* Download */}
                  ...
                  <div style={{ animation:"fadeIn 0.2s ease" }}>
                    ...
                    {user?.email === ADMIN_EMAIL && (
                      <button onClick={loadRoleAdmin} style={{ marginTop:"0.75rem", width:"100%",
                        ...
                        cursor:"pointer", color:text, fontWeight:600, fontSize:"0.85rem" }}>
                        🛡️ Panel de Roles (Admin)
                      </button>
                    )}
                  </div>
                )}
                        ...
                        {topFiles.map((f,i) => (
                          <div key={f.id} onClick={()=>{setSelectedGrade(f.grade);setSelectedSubject(f.subject);setScreen("files");}}
                            ...
                            <div style={{ textAlign:"right", flexShrink:0 }}>
                              <div style={{ color:"#4F8EF7", fontWeight:700, fontSize:"0.88rem" }}>⬇ {f.download_count||0}</div>
                            ...
                            return (
                              <div key={key} style={{ background:card, border:`1px solid ${earned?"#FBBF2444":border}`,
                                ...
                                <div style={{ fontSize:"1.8rem", marginBottom:"0.5rem",
                                  filter:earned?"none":"grayscale(1)" }}>{b.emoji}</div>
